...
% function result = SplitAndMerge(dists, angles, Pose)
% 
% argumentos:
...
delta = angles(3)*pi/180;
for i = 1:n
  X(i) = Pose.x + dists(i)*cos(Pose.th + angle + (i-1)*delta);
  Y(i) = Pose.y + dists(i)*sin(Pose.th + angle + (i-1)*delta);
endfor
